                      ...
                      TLinkedList *hashtable[], int len, TLinkedList *newentry)
{
  ...
  // int ndx = hashfun(filename, len);
  // printf("File Name: %s, Index Found: %d\n",filename, ndx);
  // hashtable[ndx] = newentry;

  int ndx = hashfun(filename, len);
  TLinkedList *node = find_llist(hashtable[ndx], filename);

  if (node != NULL)
  {
    // File already exists, replace the node
    node->filesize = newentry->filesize;
    node->startblock = newentry->startblock;
  }
  else
  {
    // File does not exist, insert the new entry
    insert_llist(&(hashtable[ndx]), newentry);
  }
}
...
TLinkedList *find_file(char *filename, int (*hashfun)(char *, int), TLinkedList *hashtable[], int len)
{
  ...
  return find_llist(filelist, filename);
}
              ...
              int len)
{
  ...
  update_hashtable(filename, hashfun, hashtable, len, new_node);

  // writelog(filename);
  // listdir(hashtable, len);
}
...
void rename_file(char *old_filename,
                 char *new_filename,
                 int (*hashfun)(char *, int),
                 TLinkedList *hashtable[], int len)
{
  ...
  // Update filename
  // strcpy(node->filename, new_filename);
  // update_hashtable(new_filename, hashfun, hashtable, len, node);
}
...
void clear_hashtable(TLinkedList *hashtable[], int len)
{
  // Implement the rest of this function below.
 for (int i = 0; i < len; i++)
    {
        ...
        while (current != NULL)
        {
            ...
            delete_llist(&hashtable[i], current);
            current = next;
        }
        hashtable[i] = NULL; // Ensure the pointer is reset to NULL
    }
